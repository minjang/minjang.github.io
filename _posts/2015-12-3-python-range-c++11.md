...
현대적 C++, 다시 말해 C++11부터 시작하여 C++14, C++17를 포함하는 C++은 과거 C++03에 비해 새로운 기능이 많이 추가되어 거의 새로운 언어가 되었다. 덕분에 파이썬 같은 언어에서나 가능했던 표현도 C++에서 이제 가능하게 되었다. 앞으로 두세 차례의 포스팅에 걸쳐 파이썬의 순환문 중 일부를 현대적 C++로 만들어 보고자 한다.

### 파이썬의 유도 변수 없는 for 순환

파이썬으로 0부터 9까지 숫자를 출력하는 순환문을 쓴다면 아래 같이 파이썬스럽게(Pythonic) 할 수 있다.
...
C++에서는 `i`, `j` 같은 변수로 순환문을 구성하는 것이 너무 당연해 보인다. 하지만 유도 변수를 감출 수 있는 순환문이 자유도를 좀 잃어도 더 안전하다. C++에서는 유도 변수의 초기화(`i = 0`), 값 비교(`i < 10`), 값 증감(`++i`)을 모두 기술해야 하지만, 파이썬은 `range`로 이 과정을 모두 가린다. 간결한 코드뿐만 아니라 실수를 줄일 수 있다.

파이썬 코드에 있는 변수 `n`은 유도 변수가 아니다. 이 변수의 유효 범위는 오직 한 번의 순환뿐이다. 아무리 `n`을 현재 순환에서 고치더라도 이 값이 다음 순환으로 전달되지 않는다. 하지만 C++ 코드의 변수 `i`는 모든 순환에 걸쳐 값이 유지되는 유도 변수이다.

유도 변수를 직접 쓰지 않는 암묵적인 순환은 예상과 다르게 최적화에도 쉬울 수 있다. C/C++ 프로그래머들은 어셈블리에 가깝게 코딩하는 것이 최적화에 가깝다고 흔히 생각하지만 틀릴 때가 많다. 사실 위 예처럼 `int i` 같은 기본 자료형 유도 변수는 컴파일러가 분석하기 쉬워서 큰 문제는 아니지만, 그래도 모든 순환 동안 살아 있는 명시적인 변수를 제거하면 컴파일러가 더욱 쉽게 최적화를 할 수 있다.

파이썬 같은 동적 언어에서는 유도 변수를 이용한 방법이 성능으로도 더 손해이므로 가능하다면 꼭 암묵적인 순환을 써야 한다. C++에서 정수형 변수의 증감과 대입은 CPU에서 가장 빠른 연산으로 실행된다. 하지만 동적 언어에서 `i`는 어떠한 타입도 될 수 있으므로 파이썬에서 `i += 1`을 하는 과정이 C++의 그것보다 훨씬 복잡하고 시간이 오래 걸린다. 타입을 먼저 검사해야 하고 타입에 맞는 덧셈과 대입 연산을 수행해야 한다. 특별한 동적 컴파일러 최적화 기술을 쓰지 않으면 이 과정은 인라인 되지 못하고 모두 함수 호출로 되어버려 C++에 비해 수백 배 이상 느려질 수 있다.

굳이 _"유도 변수는 위험하다"_, _"induction variables [considered harmful](https://en.wikipedia.org/wiki/Considered_harmful)"_ 같은 주장을 절대로 하는 것이 아니다. 유도 변수를 감추는 순환문은 분명 모든 형태의 순환문 증감이나 제어 흐름을 기술하지 못하므로 유도 변수가 필요한 곳도 있다. 가능하다면 유도 변수를 피하는 것이 좋다 정도로 받아들이면 좋겠다.


### C++에서도 비슷하게 할 수 있다

그렇다면 C++에서 파이썬스러운 코드를 작성할 수 있을까? 된다. C++11 부터 추가된 **범위 기반 for 순환** ([range-based for loop](http://en.cppreference.com/w/cpp/language/range-for))로 할 수 있다. 파이썬 처럼 행동하는 `range` C++ 함수를 만들어 유도 변수를 드러내지 않는 순환문을 아래처럼 만들 수 있다.
...
C++11 부터 재정의된 `auto`로  예를 들어 벡터 내의 원소 순환을 훨씬 간결하게 표기할 수 있다.
...
하지만 이렇게 모든 원소를 순회할 때는 앞서 말한 유도 변수/객체인 `i`를 직접 쓰지 않는 범위 기반 for 순환을 쓰면 더 깔끔하고[^1] 파이썬 코드와 거의 같은 코드를 만들 수 있다.
...
방금 예처럼 유도 변수가 C++의 순환자 객체, `std::iterator` 라면 더더욱 범위 기반 순환문이 좋다. `auto`를 이용한 예에서 순환의 끝을 미리 변수 `e`에다 저장한 것을 주목하길 바란다. `i != A.end()` 형태도 조건만 맞는다면 문제가 없어야 하지만, 혹시나 `end()`가 매번 호출되지 않을까 걱정되어 저런 코드로 쓸 때가 흔하다. 순환자의 증감 연산도 전위냐 `++i`, 후위냐 `i++`에 따라 미묘한 성능 차이가 있을 수 있다. 유도 변수가 이렇게 클래스 객체가 되니 사소한 순환에서도 신경 써야 할 점이 더 늘었다. 반면 범위 기반 순환에서는 아예 이런 순환자를 프로그래머가 신경 쓰지 않도록 하므로 컴파일러가 최적의 방법으로 코드를 생성할 수 있다. 변수 하나가 줄었으니 버그가 날 소지도 줄어든 것도 이점이다.[^2]

그렇다면 이 범위 기반 순환문의 비밀은 무엇인가? 혹시 STL 객체하고만 작동할까? 아니다. 일반 배열도 되므로 이건 아닐 것이다. 답은 의외로 간단하다. 사실 그냥 간편 문법(syntactic sugar)에 지나지 않는다.


C++ 표준[^3]에 따르면 범위 기반 for 순환 문법은
...
요약하면, 특별한 마법이 있는 것이 아니라 순환의 시작과 끝을 `begin`과 `end` 메서드/함수로 얻을 수 있고, 전위 증가, `!=`, 참조(`*`) 연산자만 있으면 범위 기반 for 순환을 쓸 수 있다는 뜻이다. 이 배경 지식으로 이제 `range`를 설계해보자.
...
본격적으로 설계할 것은 `range` 함수가 아니라 `something`에 해당하는 클래스이다. `range` 함수는 단지 `something` 클래스를 생성해서 돌려줄 뿐이다. 이 클래스 이름을 짓기 전에 범위 기반 for 순환이 어떻게 변환되는지 직접 `range(0, 10)`을 대입해보자. 그러면 아래 코드가
...
`range_impl`의 주요 임무는 `range` 함수로부터 시작과 끝 인덱스를 받아 기억하고 시작과 끝 순환자를 반환하는 것이다.
...
C++11부터 시작된 현대 C++의 새로운 기능으로 파이썬, 루비 같은 언어에서나 가능했던 표기도 점점 가능해지고 있다. 파이썬의 `range` 함수를 예제로 왜 유도 변수를 직접 쓰지 않는 순환문이 좋은지 먼저 이야기했다. 그리고 C++11의 범위 기반 순환문으로 이를 구현해보았다. 표준 문서에 근거해 범위 기반 순환이 어떻게 정의되는지를 알고 나니 구현은 뜻밖에 간단하였다.

#### 더 생각해볼 문제

- 과연 성능은 어떠할까? 순환자 클래스의 오버헤드가 얼마나 될까?
- _step_ 기능을 구현한다면? 관심 있는 분은 연습 문제로 풀어보면 재밌을 것이다.
- 파이썬에는 `enumerate`라는 순환문 지원 함수가 있다. 이것 역시 현대 C++로 만들 수 있다. 아마도 다음 글에서 이 이야기를 할 것 같다.

[^1]: 어떨 때는 아예 `for` 순환 자체를 감춰버리는 [std::accumulate](http://en.cppreference.com/w/cpp/algorithm/accumulate) 같은 알고리즘을 쓸 수도 있다.
[^2]: 하지만 이 블로그의 제목처럼 악마는 디테일에 있다고 범위 기반 순환에서  실수를 저지를 수도 있다. 이 이야기는 나중에 기회가 되면 하고 관심있는 독자는 [이 글](http://www.open-std.org/JTC1/SC22/WG21/docs/papers/2014/n3853.htm)을 읽어 보길 바란다.
